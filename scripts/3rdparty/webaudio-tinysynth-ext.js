...
function WebAudioTinySynthCore(target) {
  Object.assign(target,{
    properties:{
      ...
      synthType:  {type: String, value:'internal'}, // {'internal', 'external'}
      externalDevice: {type: Object, value: {}},
      timeStarted: {type: Number, value: 0},
      /*@@gui*/
...
*/
    ...
    setExternalDevice: (obj={}) => {
      this.externalDevice = obj
      console.log(this.externalDevice)
    },
    setTimeStartedZero: () => {
      this.timeStarted = 0
    },
    setSynthType: (type='internal') => {
      if (type.match(/^internal|external$/)==null) {
        type = 'internal'
      }
      this.synthType = type // 'external'
    },
    send:(msg,t)=>{    /* send midi message */
      if (this.synthType=='external') {
        this.externalDevice.send(msg, this.timeStarted + t)
        return
      }
      const ch=msg[0]&0xf;
      const cmd=msg[0]&~0xf;
