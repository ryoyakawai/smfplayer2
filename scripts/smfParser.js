...
export class SmfParser {
  ...
  parse(data) {
    let stream=this.Stream(data)
    let tracks=[]
    let channels=[[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[]]
    let systems=[]
    let headerChunk = this.readChunk(stream)

    if (headerChunk.id != 'MThd' || headerChunk.length != 6) {
      throw "Bad .mid file - header not found"
    }
    let headerStream = this.Stream(headerChunk.data)
    ...
    for (let i = 0; i < header.trackCount; i++) {
      tracks[i] = []
      this.totalTime=0
      let trackChunk = this.readChunk(stream)
      if (trackChunk.id != 'MTrk') {
        throw "Unexpected chunk - expected MTrk, got "+ trackChunk.id
      }
      let trackStream = this.Stream(trackChunk.data)
      this.totalTime=0
      while (!trackStream.eof()) {
        let event = this.readEvent(trackStream)
        if(event!=null) {
          tracks[i].push(event)
    ...
    if ((eventTypeByte & 0xf0) == 0xf0) {
      ...
      switch (eventType) {
        ...
        case 0x0e:
          ...
          let msb=event.value & 0x7f
          let lsb=event.value >>> 7
